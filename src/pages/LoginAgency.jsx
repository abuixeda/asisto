...
export default function LoginAgency() {
  ...
  async function handleLogin(e) {
    e.preventDefault();
    setError('');
    if (!email || !password) { setError(t('loginAgency.errors.fillAll')); return; }
    setLoading(true);
    try {
      ...
      if (!res.ok) { setError(data.error || t('loginAgency.errors.default')); return; }
      ...
      setError(t('loginAgency.errors.connection'));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.bg, padding: '2rem', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '2.5rem', width: '100%', maxWidth: '440px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div onClick={() => nav('/premium')} style={{ display: 'flex', alignItems: 'baseline', gap: '0.4rem', cursor: 'pointer' }}>
        ...
        <h2 style={{ margin: '0 0 0.5rem 0', fontSize: '1.6rem', color: C.text, fontWeight: 700 }}>{t('loginAgency.title')}</h2>
        <p style={{ color: C.textSec, margin: '0 0 2rem 0', fontSize: '0.95rem' }}>{t('loginAgency.subtitle')}</p>

        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.9rem', color: C.textSec, fontWeight: 500 }}>{t('loginAgency.email')}</label>
            <input
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
              <label style={{ fontSize: '0.9rem', color: C.textSec, fontWeight: 500 }}>{t('loginAgency.password')}</label>
              <a href="/olvide-contrasena" style={{ fontSize: '0.82rem', color: C.accent, textDecoration: 'none' }}>{t('loginAgency.forgotPassword')}</a>
            </div>
            <input
              type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder={t('loginAgency.passwordPlaceholder')}
              style={{ width: '100%', padding: '0.85rem 1rem', borderRadius: '10px', border: `1px solid ${C.border}`, background: '#f8fafc', color: C.text, fontSize: '1rem', boxSizing: 'border-box', outline: 'none', transition: 'border-color 0.2s' }}
              onFocus={(e) => e.target.style.borderColor = C.accent}
              onBlur={(e) => e.target.style.borderColor = C.border}
            />
          ...
          >
            {loading ? t('loginAgency.loading') : t('loginAgency.submit')}
          </button>
        </form>
